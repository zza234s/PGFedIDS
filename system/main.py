...
from flcore.servers.serverltiids import FedLTIIDS
from flcore.servers.serverltiids_plus import FedLTIIDS_Plus
...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        elif model_str=='cicids':
            if "cicids" in args.dataset:
                args.model = CICIDS_Transformer().to(args.device)
            elif 'NSLKDD' in args.dataset:
                args.model=CICIDS_Transformer(in_feature=122, out_features=5, d_model=122).to(args.device)
        ...
        if args.algorithm == "FedAvg":
            ...
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = FedLTIIDS(args, i)
        elif args.algorithm == 'ours_plus':
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = FedLTIIDS_Plus(args, i)
    ...
    # Global average
    average_data(dataset=args.dataset, algorithm=args.algorithm, goal=args.goal, times=args.times, metrics=['rs_test_acc','re_IDS_indices'])
...
if __name__ == "__main__":
    ...
    # SCAFFOLD / FedGH
    parser.add_argument('-slr', "--server_learning_rate", type=float, default=0.1) #default:1.0, FedGH 0.1--0.6689 ; 0.5
    # FedALA
    ...
    parser.add_argument('-klw', "--kl_weight", type=float, default=0.001)

    # ours
    parser.add_argument('-en_lr', "--ensemble_lr", type=float, default=0.01)
    parser.add_argument('-s_c', "--p_sim_cal", type=bool, default=True)
    parser.add_argument('-use_en', "--use_ensemble", type=bool, default=True)
    ...
    print(f"use use_prototype_ensemble：{args.use_ensemble}")
